...
class SmobListRepository(
    ...
    /**
     * Insert several smob lists in the db. Replace any potentially existing smob list record.
     ...
     */
    override suspend fun updateSmobList(smobListATO: SmobListATO) =
        withContext(ioDispatcher) {
            // support espresso testing (w/h coroutines)
            wrapEspressoIdlingResource {
                smobListDao.updateSmobList(smobListATO.asDatabaseModel())
            }
        }
     ...
     */
    override suspend fun refreshSmobListDataInDB() {
...
//            val response2: SmobListDTO? = getSmobListFromApi("07c295ad-b286-41f7-b2ea-e81a75875d02")
//            Timber.i(response2?.toString())
//
//            val testTxList: SmobListDTO = SmobListDTO(
//                listname = "maMu",
//                name = "Max Mustermann",
//                email = "max@mustermann.org",
//                imageUrl = null
//            )
//            saveSmobListViaApi(testTxList)
//
//            // read back 'first' max mustermann
//            testTxList.imageUrl = Date().time.toString()
//            updateSmobListViaApi(
//                "0794b744-7fa4-4440-b284-8c72012ed6cf",
//                testTxList
//            )
//
//            deleteSmobListViaApi("1bbd2da1-e028-4e42-b6b8-7d944013abca")
    ...
    private suspend fun getSmobListViaApi(id: String): SmobListDTO? = withContext(ioDispatcher) {
        ...
        wrapEspressoIdlingResource {
            ...
            return@withContext try {
                // return successfully received data object (from Moshi --> PoJo)
                responseHandler.handleSuccess(smobListApi.getSmobListById(id))
                    .data
                    ?.body()
                    ?.asRepoModel()    // returns 'null' if requested list cannot be found
