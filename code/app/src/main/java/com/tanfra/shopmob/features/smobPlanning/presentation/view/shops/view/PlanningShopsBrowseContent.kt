...
@Composable
fun PlanningShopsBrowseContent(
    viewState: PlanningViewState,
    preFilteredItems: List<SmobShopATO>,
    onSwipeActionConfirmed: (SmobShopATO) -> Unit,
    onSwipeIllegalTransition: () -> Unit,
    onClickItem: (SmobShopATO) -> Unit,
    onReload: () -> Unit,
) {
    Column(
        ...
        if (viewState.isContentVisible) {
            PlanningShopsBrowseView(
                snackbarHostState = viewState.snackbarHostState,
                shops = preFilteredItems,
...
@Composable
fun PreviewPlanningShopsBrowseContent() {

    val dE1 = SmobShopATO(status = ItemStatus.IN_PROGRESS)
    val dE2 = SmobShopATO(status = ItemStatus.DONE)
