...
/**
 * Interface for local Data Access Object for the smobProducts table.
 */
@Dao
interface SmobProductLocalDataSource: SmobItemLocalDataSource<SmobProductDTO> {

    /**
     * @param smobItemId the ID of the smob product
     * @return the smob product object with the smobItemId - or null if the table is empty
     */
    //
    ...
    /**
     * @return all smobProducts - returns an empty list, if the table is empty
     */
    //
    // note: Flow types must not be declared as "suspend"able functions, see the third answer in:
    //       https://stackoverflow.com/questions/46445964/room-not-sure-how-to-convert-a-cursor-to-this-methods-return-type-which-meth
    @Query("SELECT * FROM smobProducts ORDER BY productId ASC")
