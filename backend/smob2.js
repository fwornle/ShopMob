...
function smob () {
    ...
    for (let id = 0; id < daLists.length; id++) {
        ...
        // define some local shops
        const daPlans = [
            { name: 'Rewe', category: 'SUPERMARKET', latitude: faker.address.latitude(), longitude: faker.address.longitude() },
        ]

    // generate floor plan(s)
    var smobPlans = []
    var planIds = []
    for (let id = 0; id < daPlans.length; id++) {
